...
{
    /* This class contains seeding of the Database. It adds some custom stocks 
     * to the Stocks table in Stocks.db. It also adds an Admin user with salted
     * and hashed password.
     */
    public class DBInit
    {
        ...
        public static async void init(IApplicationBuilder app)
        {
            using (var serviceScope = app.ApplicationServices.CreateScope())
            {
                ...
                var storefjellHotell = new Stock("Storefjell Hotell", 600,
                    400000, 8);
                ...
                var utdanningsdirektoret = new Stock("Utdanningsdirektoratet",
                    5000, 300000000, 44);
                var colorLine = new Stock("Color Line", 30000, 3600000, -33);
                var dde = new Stock("D.D.E", 5000, 4000000, 3);
                var tantrasenteret = new Stock("Tantrasenteret Oslo", 200,
                    4000, 1);
                var rettInnBar = new Stock("Rett Inn Bar", 1, 35000, -30);
                var kongsbergGruppen = new Stock("Kongsberg Gruppen", 9000,
                    1500000000000, 780);
                var russland = new Stock("Russland", 0.01, 20000, -3400);
                var sleepyJoe = new Stock("Joe Biden", 3700, 40000000, -370);

                var quackAttack = new User { Firstname = "Quacken",
                    Surname = "Attacken", Address = "Gølle", 
                    PostalCode = "2918", PostalOffice = "Ulnes",
                    Username = "Quacky", Password = "Ranheim123" };
